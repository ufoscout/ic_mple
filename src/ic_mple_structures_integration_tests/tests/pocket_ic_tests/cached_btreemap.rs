...
async fn should_init_tx_cached_btreemap() {
    ...
    assert!(ctx.get_tx_from_cached_btreemap(0).await.is_some());
}
...
async fn should_persist_cached_btreemap_tx_after_upgrade() {
    let ctx = new_test_context().await;
    ctx.insert_tx_to_cached_btreemap(1, 1, 10).await;

    assert!(ctx.get_tx_from_cached_btreemap(1).await.is_some());
