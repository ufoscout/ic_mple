...
async fn should_init_tx_ring_buffer() {
    ...
    let res = ctx.get_tx_from_ring_buffer(0).await;
    assert!(res.is_some());
}
...
async fn should_push_tx_to_ring_buffer() {
    let ctx = new_test_context().await;
    ctx.push_tx_to_ring_buffer(1, 1, 10).await;

    assert!(ctx.get_tx_from_ring_buffer(1).await.is_some());
}
...
async fn should_persist_ring_buffer_tx_after_upgrade() {
    ...
    assert!(ctx.get_tx_from_ring_buffer(0).await.is_some());
    assert!(ctx.get_tx_from_ring_buffer(1).await.is_some());
}
